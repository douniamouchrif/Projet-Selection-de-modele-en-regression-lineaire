...
bp2_jambe_m <- ggplot(X_m, aes(x=factor("."), y=MhauteurGenouDeboutJambe)) + labs(title="Boite à moustaches de la Hauteur Genou Debout Jambe",
  ...
  coord_cartesian(ylim = c(275,500))
...
Dans le but de trouver un modèle pour expliquer la variable poid, nous avons utilisé plusieurs méthodes.
...
#on retient 20 variables pour les hommes et 15 pour les femmes
```


# Premère façon : AIC (méthode desendante)

###HOMMES
```{r}
# Modèle complet avec 20 variables
model_complet_m <- lm(Y_m ~ .,data=X_m_selected)
model_backward_m <- stepAIC(model_complet_m,direction="backward")
#on obtient un modèle à 15 variables 

```

###FEMMES
```{r}
# Modèle complet avec 15 variables
model_complet_f <- lm(Y_f ~ .,data=X_f_selected)
model_backward_f <- stepAIC(model_complet_f,direction="backward")
#on obtient un modèle à 11 variables
```


# Deuxième façon : lm() à la main

## On part de la totalité des 92 variables
```{r}
'library(car)
mod = lm...  
vid(mod)'

mod_m1 <- lm(Y_m ~ . , data=X_m)
#summary(mod_m1)

mod_m2 <- lm(Y_m ~ abdominalextensiondepthsitting + acromialheight + acromionradialelength + 
              anklecircumference + axillaheight + balloffootcircumference +
              bicepscircumferenceflexed + bicristalbreadth + bitragionchinarc +
              ...
              crotchheight + crotchlengthomphalion + earlength + elbowrestheight + 
              footbreadthhorizontal + footlength + forearmcircumferenceflexed +
              forearmhandlength + headcircumference + headlength + heelanklecircumference +
              heelbreadth + hipbreadth + hipbreadthsitting + iliocristaleheight +
              interscyei + kneeheightmidpatella + kneeheightsitting +
              lateralfemoralepicondyleheight + lowerthighcircumference + neckcircumference +
              palmlength + poplitealheight + shouldercircumference + sittingheight + 
              stature + tenthribheight + thighcircumference + thighclearance + tibialheight +
              verticaltrunkcircumferenceusa + waistcircumference + waistdepth +
              waistheightomphalion + wristcircumference + wristheight, data=X_m)
#summary(mod_m2)

mod_m3 <- lm(Y_m ~ abdominalextensiondepthsitting + acromialheight + acromionradialelength + 
              anklecircumference + axillaheight + balloffootcircumference +
              bicepscircumferenceflexed + bicristalbreadth + bitragionchinarc +
              buttockcircumference + buttockdepth + buttockheight + buttockkneelength +
              calfcircumference + chestbreadth + chestcircumference + chestdepth +
              ...
              forearmhandlength + headcircumference + headlength + heelanklecircumference +
              heelbreadth + hipbreadth + hipbreadthsitting + iliocristaleheight +
              kneeheightsitting + lateralfemoralepicondyleheight + lowerthighcircumference +
              neckcircumference + palmlength + poplitealheight + shouldercircumference +
              sittingheight + stature + tenthribheight + thighcircumference + 
              thighclearance + tibialheight + verticaltrunkcircumferenceusa +
              waistcircumference + waistdepth + waistheightomphalion + wristcircumference +
              wristheight, data=X_m)
#summary(mod_m3)
```

Conclusion pour cette façon : pas pertinent de le faire à la main en partant de la totalité des variables car on part d'un très gros nombre (92) et cela devient très vite illisible et perte de temps lorsqu'il existe déjà des fonctions pouvant trouver de meilleurs modèles tout en étant pertinent.

## On part d'un nombre de variables réduit, qui ont chacune une corrélation supérieure à 0,8 avec la variable Poids.

###HOMMES
```{r}
mod_m4 <- lm(Y_m ~ . , data = X_m_selected)
#summary(mod_m4)

mod_m5 <- lm(Y_m ~ buttockcircumference + buttockdepth + calfcircumference +
               chestcircumference + chestdepth + forearmcircumferenceflexed +
               forearmforearmbreadth + hipbreadth + hipbreadthsitting +
               lowerthighcircumference + shouldercircumference + thighclearance +
               verticaltrunkcircumferenceusa + waistcircumference + waistdepth, 
             data = X_m_selected)
#summary(mod_m5)

mod_m6 <- lm(Y_m ~ buttockcircumference + buttockdepth + calfcircumference +
               chestcircumference + chestdepth + forearmcircumferenceflexed +
               hipbreadth + hipbreadthsitting + lowerthighcircumference +
               shouldercircumference + thighclearance + verticaltrunkcircumferenceusa +
               waistcircumference + waistdepth, data = X_m_selected)
summary(mod_m6)
```

###FEMMES
```{r}
mod_f1 <- lm(Y_f ~ . , data = X_f_selected)
#summary(mod_f1)

mod_f2 <- lm(Y_f ~ bicepscircumferenceflexed + buttockcircumference + chestcircumference +
               forearmcircumferenceflexed + hipbreadthsitting + lowerthighcircumference +
               shouldercircumference + thighclearance + verticaltrunkcircumferenceusa +
               waistcircumference, data = X_f_selected)
summary(mod_f2)
```
...
Nous allons maintenant chercher quel modèle est le meilleur parmis celui trouvé par le critère de l'AIC et celui trouvé par la méthode de régression lm() pour chaque genre pour expliquer le poid. 

```{r}
# Comparaison des modèles hommes
if (BIC(model_backward_m) < BIC(mod_m6)) {
  print("Le modèle trouvé par le critère AIC est préférable pour les hommes.")
} else {
  print("Le modèle trouvé par la methode de régression lm() est préférable pour les hommes")
}

# Comparaison des modèles femmes
if (BIC(model_backward_f) < BIC(mod_f2)) {
  print("Le modèle trouvé par le critère AIC est préférable pour les femmes")
} else {
  print("Le modèle trouvé par la methode de régression lm() est préférable pour les femmes")
}
...
```{r}
#peut être faire les courbes roc pour chaque modèle ?? comme le prof demande des grphiques et tableau ?
#peut etre separer nos données en 2 echantillon test et train ?? psk la on a forcement une aire de 1 sous la courbe
#jsp si c'est pertiant mais comme il veux des graphique je vois pas quoi faire d'autre 

prediction <- predict(model_backward_m, X_m)
library(pROC)
roc_curve <- roc(Y_m, prediction)
auc(roc_curve)
plot(roc_curve)
```
