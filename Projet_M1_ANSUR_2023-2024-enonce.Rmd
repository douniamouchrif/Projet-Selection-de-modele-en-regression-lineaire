...
main <- na.omit(main)
imain <- which(!is.na(match(main, names(X_m))))
names(X_m)[imain] <- c("MlargeurMain", "McironférenceMain", "MlongueurMain", "MlongueurPaumeMain")
...
main <- c("handbreadth", "handcircumference", "handlength", "palmlength")
main <- na.omit(main)
imain <- which(!is.na(match(main, names(X_f))))
names(X_f)[imain] <- c("FlargeurMain", "FcironférenceMain", "FlongueurMain", "FlongueurPaumeMain")
...
print('coucou dounia')
####HOMMES
statureM <- c("overheadfingertipreachsitting",
...
statureM <- na.omit(statureM)
istatureM <- which(!is.na(match(statureM, names(X_m))))
colnames(X_m)[istatureM] <-
   ...
   "MhauteurBasFesseStature",
   "MhauteurBoutFesseStature",
   "MhauteurHautFesseStature",
   "MtailleStature", "MhauteurCouStature", "MhauteurÉpauleStature", "MHauteurPoignetStature")
stature_names_m <- names(X_m)[grep("Stature", names(X_m))]
statureM_cor = cor(statureM)
corrplot(StatureM_cor)

####FEMMES
 
statureF <- c("overheadfingertipreachsitting",
   "elbowrestheight", "eyeheightsitting", "sittingheight",
   "axillaheight", "chestheight", "iliocristaleheight",
   "tenthribheight", "suprasternaleheight", "waistheightomphalion",
   "crotchheight",
   ...
   "trochanterionheight",
   "stature", "cervicaleheight", "acromialheight", "wristheight")
statureF <- na.omit(statureF)
istatureF <- which(!is.na(match(statureF, names(X_f))))
colnames(X_f)[istatureF] <-
   c("MassisPlafondStature",
   ...
   "FhauteurBasFesseStature",
   "FhauteurBoutFesseStature",
   "FhauteurHautFesseStature",
   "FtailleStature", "FhauteurCouStature", "FhauteurÉpauleStature", "FhauteurPoignetStature")
stature_names_f <- names(X_f)[grep("Stature", names(X_f))]

statureF <- (X_m[, stature_names_f])
statureF_cor = cor(statureF)
corrplot(StatureF_cor)
```
