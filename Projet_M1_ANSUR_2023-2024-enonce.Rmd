...
```{r}
library(MASS)
# Modèle complet avec 92 variables (trop long)
#model_complet_m <- lm(Y_m ~ .,data=X_m)
#stepAIC(model_complet,direction="backward")

####HOMMES
# Modèle complet avec 20 variables
model_complet_m <- lm(Y_m ~ .,data=X_m_selected)
stepAIC(model_complet_m,direction="backward")
...
stepAIC(model_complet_f,direction="backward")
...
if (BIC(modele1) < BIC(modele2)) {
  print("Le modèle 1 est préférable.")
} else {
  print("Le modèle 2 est préférable.")
}'
```


# Troisième façon : lm() à la main
...
selected_columns_f <- which(correlations_f > 0.8)
X_f_selected <- X_f[, selected_columns_f]
# on supprimera les codes du dessus après
```
